...
//リレーの状態
enum {
	POWER_OUTPUT_LOW, POWER_OUTPUT_HIGH
};
...
static void MX_GPIO_Init(void) {
	...
	OUTPUT_current_reset_Pin | OUTPUT_POWER_SD_Pin | OUTPUT_LED_G_Pin,
			GPIO_PIN_RESET);

	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOB, OUTPUT_LED_B_Pin | OUTPUT_LED_R_Pin,
			GPIO_PIN_RESET);
	...
	GPIO_InitStruct.Pin = OUTPUT_current_reset_Pin | OUTPUT_POWER_SD_Pin
			| OUTPUT_LED_G_Pin;
